...
description: Edit audio or video podcast recordings through transcript-led cuts, two user-editable Markdown plans, cold-open highlights and music, dialogue repair, multilingual subtitle files, self-media cover art, mastering, holistic AI review, and platform publishing metadata. Use for MP4, MOV, MKV, WAV, M4A, MP3, reference episodes, and podcast release packages. Route video sources to audio-led or full-visual editing; route audio-only sources to podcast delivery without video-specific YouTube or Xiaohongshu assets.
---
...
- Optional visual route: an authorized Google Photos/Drive connector, licensed media search, HTML/canvas workflow, `image_gen.imagegen`, or an available video-generation function. Discover connector/generator calls through the current tool registry; use them only in `full-visual` mode and only after authorization, except that a requested social-media cover may use an authorized image-generation or design route for any source type.
...
- `audio-only`: edit and master the podcast audio. Skip video planning, video exports, YouTube copy, and Xiaohongshu copy. A requested social-media cover is the sole visual deliverable allowed; create podcast-feed metadata only when publishing materials are requested.
...
3. Create or refresh `output/episode-brief.md` from `assets/templates/episode-brief.md`. Require only the core purpose; source type; processing route; exclusions; target platforms/duration; references; subtitle languages/formats; cover requirements; and other non-negotiables. Do not require intended audience effect, desired tone, or must-keep material.
...
7. When publishing materials or a cover are requested, create `output/cover-brief.md` from `assets/templates/cover-brief.md`. Inspect relevant visual references in `assets/` first (for example, prior covers, logos, fonts, palettes, layouts, and brand guides) and use them as the primary style reference. Otherwise, derive a clean platform-appropriate direction from the episode and clearly record it. Do not copy a third party's artwork, use unlicensed portraits/logos, or put unsupported claims on the cover.
...
5. Create the approved self-media cover for each requested aspect ratio/platform. Preserve the episode's visual references from `assets/` when provided; otherwise use the approved cover brief. Export a legible PNG or JPG and retain editable source/design files when the chosen tool supports them. Do not use generated likenesses of real people without authorization.
6. Render a low-cost preview for nontrivial or uncertain edits. When the plan is deterministic and explicitly approved, proceed directly to a delivery render and QC to save time.
7. Record changes in `output/edit-log-v1.md`; keep prior versions.
...
5. Generate publishing materials only for requested platforms. For audio-only input, generate podcast-feed title, summaries, chapters, tags/categories, credits, and manifest; never generate YouTube or Xiaohongshu video assets. A requested social-media cover remains allowed for any source type.
6. Inspect each delivered cover at final pixel dimensions: confirm the intended platform/aspect ratio, readable title and faces at thumbnail size, safe margins, accurate names/claims, reference-style consistency, and image/source rights. Record the cover paths, dimensions, reference assets, and generation/design method in `output/qc-report.md` and `output/asset-log.md`.
7. Validate media/subtitle paths, JSON, chapters, names, and factual claims. Create `output/qc-report.md`.
